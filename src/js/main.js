...
class App extends React.Component {
  constructor() {
    super()
    this.state = { tab: 'radar' }
  }
  componentDidMount() {
    dragElement(this.artdiv)
    this.artdiv.style.top = document.documentElement.scrollTop + 30;
    this.artdiv.style.left = Math.round(window.innerWidth / 2) - 30;
    document.addEventListener("keyup", handleKeyUp, false);
  }
  render() {
    const routedTab = { 'radar': <Radar />, 'traps': <Traps /> }[this.state.tab]
    return (
      <div id="asyncRenderToolbox_div" ref={e => this.artdiv = e}>
        <div id="asyncRenderToolbox_divheader">
          async-render-toolbox (alt/⌥+R to toggle)
        </div>
        <RouteBar>
          <Btn onClick={() => this.setState({ tab: 'radar' })} disabled={this.state.tab === 'radar'}>radar</Btn>
          <Btn onClick={() => this.setState({ tab: 'traps' })} disabled={this.state.tab === 'traps'}>traps</Btn>
        </RouteBar>
        <div id="asyncRenderToolbox_body" ref={e => this.artbody = e}>
          {routedTab}
        </div>
      </div>
    )
